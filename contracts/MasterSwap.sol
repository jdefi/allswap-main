...
// MasterSwap is the master of Mega. He can make Mega and he is a fair guy.
//
...
contract Masterswap is Ownable {
    ...
    function setMigrator(IMigratorSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function pendingMega(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMegaPerShare = accMegaPerShare.add(megaReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accMegaPerShare = pool.accMegaPerShare.add(megaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to Masterswap for MEGA allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMegaPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    // Withdraw LP tokens from Masterswap.
